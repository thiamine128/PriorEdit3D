...
# AMP decorators:
# - Newer PyTorch prefers torch.amp.custom_fwd/custom_bwd(device_type="cuda")
# - torch.cuda.amp.custom_* is deprecated in newer versions (FutureWarning)
# Keep behavior compatible across versions by feature-detecting instead of relying purely on version checks.
try:
    import torch.amp as amp  # PyTorch >= 2.0
    try:
        _TORCH_CUSTOM_FWD = amp.custom_fwd(cast_inputs=torch.float16, device_type="cuda")
    except TypeError:
        # Older torch.amp.custom_fwd without device_type kwarg
