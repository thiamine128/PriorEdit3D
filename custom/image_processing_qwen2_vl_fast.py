...
class Qwen2VLImageProcessorFast(BaseImageProcessorFast):
    ...
    # +++ Mimics temporal logic (T=1) like the non-fast variant   +++
    # +++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++
    def process_precomputed_tensors(
        ...
        """
        Processes pre-computed image tensors (e.g., [B, C, H, W]) directly into flattened patches
        using PyTorch, preserving gradients.

        This matches the custom behavior added in `image_processing_qwen2_vl.py`:
        - Treat each item as a single-image input (T=1 in `image_grid_thw`)
        - Internally simulates temporal padding to `temporal_patch_size` for patch-dim construction

        ASSUMPTION: `image_tensors` are already resized, rescaled, and normalized.
        """
        ...
        # --- Dimension checks ---
        if H % p != 0 or W % p != 0:
            raise ValueError(f"Input tensor H ({H}) and W ({W}) must be divisible by patch_size ({p})")
        grid_h = H // p
        grid_w = W // p
        if grid_h % m != 0 or grid_w % m != 0:
            raise ValueError(f"Grid H ({grid_h}) / W ({grid_w}) must be divisible by merge_size ({m})")
        ghm = grid_h // m
        gwm = grid_w // m

        # Force T=1 grid logic (same as passing a single image through the non-fast processor)
        num_frames_per_item = 1
        padded_num_frames_per_item = num_frames_per_item
        if num_frames_per_item % tp != 0:
            padded_num_frames_per_item += tp - (num_frames_per_item % tp)
        target_grid_t = padded_num_frames_per_item // tp  # should be 1

        # Spatial patch extraction
        # [B, C, H, W] -> [B, C, ghm, m, p, gwm, m, p]
        ...
        # Simulate temporal padding within patch dimension: final dim is C * tp * p * p
        spatial_patches_flat_pp = spatial_patches_flat.view(B * grid_h * grid_w, C, p * p)
        repeated_temporal = spatial_patches_flat_pp.unsqueeze(2).repeat(1, 1, tp, 1)
        flatten_patches = repeated_temporal.contiguous().view(B * grid_h * grid_w, C * tp * p * p)

        image_grid_thw = torch.tensor([target_grid_t, grid_h, grid_w], dtype=torch.long, device=device).unsqueeze(0)
        image_grid_thw = image_grid_thw.repeat(B, 1)  # [B, 3]

        return BatchFeature(data={"pixel_values": flatten_patches, "image_grid_thw": image_grid_thw}, tensor_type=return_tensors)
